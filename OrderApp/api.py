...
class App_api:
    # return functional code start (process return request from shopify page(return) )#
    @staticmethod
                            ...
                            if (request.GET.get('variants') is not None) and (request.GET.get('quantity') is not None): # check the requested variant and quantity is not none or blank #
                                ...
                                if get_rec_data_read:   # data is  match then return an error #
                                    ...
                                    if count == 999:    # for single data in request get variant, quantity and reason_note #
                                        ...
                                        request_vari_data = requested_variant.objects.create(order_number=or_id, cust_id=cus_id, variants=variant_data, r_status=data_status, varaint_shop=data_shop, quantity=quant_data, status=get_rec_data.order_fulfill_status, order_reason=reason_data, order_note="", crea_date=date_data, upda_date=date_data, detail_order_id_id=data_detail)
                                    ...
                                    if datadmin: # if SuperAdmin data get #
                                        ...
                                        get_data['logo'] = "https://6developer3i.pagekite.me/media/" + str(datadmin[0].Imageurl)
                                        ...
                                        if data_thanks: # filter success #
                                            ...
                                            logo = "https://6developer3i.pagekite.me/media/" + data_thanks[0].logo_upload
